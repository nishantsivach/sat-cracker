...
function LoginForm() {
  ...
  return (
    <div className="relative min-h-screen flex overflow-hidden">
      ...
      <div className="hidden md:flex w-1/2 items-center justify-center p-10 lg:p-16">
        <div className="max-w-md">
            ...
            {[
              {
                ...
                text: "AI-powered explanations that click",
              },
              {
                icon: <CheckCircle2 className="w-4 h-4" />,
                text: "Adaptive practice for your weak spots",
              },
              {
                icon: <CheckCircle2 className="w-4 h-4" />,
                text: "Track progress toward your goal score",
              },
            ].map((item, i) => (
              <div
                key={i}
                className="flex items-center gap-3.5 text-site-text group cursor-default"
              >
          ...
          <div className="mt-8 flex items-center gap-3 text-xs text-site-muted">
            ...
            <span>
              Joined by{" "}
              <span className="font-semibold text-site-text">25,000+</span>{" "}
              students
            </span>
          </div>
        </div>
      ...
      {/* RIGHT — Navy gradient background with glass card */}
        ...
        <div className="relative w-full max-w-sm bg-white/10 backdrop-blur-2xl rounded-3xl border border-white/15 shadow-2xl shadow-black/20 overflow-hidden">
          ...
          <div className="relative z-10 p-7 md:p-8">
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2.5 border border-white/15 bg-white/10 backdrop-blur-sm py-2.5 rounded-xl font-semibold text-sm text-white hover:bg-white/20 hover:border-white/25 hover:shadow-lg hover:-translate-y-0.5 transition-all group"
            >
            ...
            <form onSubmit={handleEmailLogin} className="space-y-3.5">
              ...
              <div>
                ...
                >
                  ...
                  <input
                    ...
                    onBlur={() => setFocusedField(null)}
                    className="w-full pl-10 pr-12 py-2.5 rounded-xl border border-white/10 bg-white/5 backdrop-blur-sm text-white placeholder:text-white/25 focus:outline-none focus:ring-4 focus:ring-site-accent/20 focus:border-site-accent/40 focus:bg-white/10 transition-all text-sm"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-3.5 top-1/2 -translate-y-1/2 text-white/40 hover:text-white/70 transition-colors p-1 rounded-lg hover:bg-white/10"
                  >
                    {showPassword ? (
              ...
              <div className="flex justify-between items-center pt-0.5">
                ...
                <Link
                  ...
                  className="text-[11px] text-site-accent hover:text-amber-300 font-medium transition-colors"
                >
                  Forgot password?
                </Link>
              </div>
