    ...
    <Layout>
      ...
      <section className="max-w-3xl mx-auto px-6 py-12">
        <div className="grid grid-cols-3 gap-4 mb-10">
          <div className="bg-white rounded-xl border border-site-border p-4 text-center">
            <BookOpen className="w-5 h-5 text-site-secondary mx-auto mb-2" />
            <p className="text-xl font-black text-site-text">
              {modules?.length || 0}
            </p>
            <p className="text-[11px] text-site-muted">Modules</p>
          </div>
          <div className="bg-white rounded-xl border border-site-border p-4 text-center">
            <PlayCircle className="w-5 h-5 text-site-secondary mx-auto mb-2" />
            <p className="text-xl font-black text-site-text">{totalLessons}</p>
            <p className="text-[11px] text-site-muted">Lessons</p>
          </div>
          <div className="bg-white rounded-xl border border-site-border p-4 text-center">
            <Sparkles className="w-5 h-5 text-site-accent mx-auto mb-2" />
            <p className="text-xl font-black text-site-text">{freeLessons}</p>
            <p className="text-[11px] text-site-muted">Free previews</p>
          </div>
        </div>

        <div className="space-y-4">
          {modules?.map((module, moduleIndex) => {
            const lessons = [...(module.lesson || [])].sort(
              (a, b) => a.order - b.order
            );
            const completedInModule = 0; // TODO: connect to user progress
            const moduleNumber = String(moduleIndex + 1).padStart(2, "0");

            return (
              <div
                key={module.id}
                className="bg-white rounded-2xl border border-site-border overflow-hidden hover:border-site-accent/10 transition-colors"
              >
                {/* Module header */}
                <div className="px-6 py-5 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <span className="text-xs font-mono font-bold text-site-accent bg-site-highlight rounded-lg px-2.5 py-1.5">
                      {moduleNumber}
                    </span>
                    <div>
                      <h2 className="font-bold text-site-text">
                        {module.title}
                      </h2>
                ...
                <div className="border-t border-site-border">
                  {lessons.map((lesson, lessonIndex) => (
                    <Link
                      ...
                      className={`flex items-center justify-between px-6 py-3.5 hover:bg-site-highlight/50 transition-colors ${
                        lessonIndex < lessons.length - 1
                          ? "border-b border-site-border/50"
                          : ""
                      }`}
                    >
                      <span className="flex items-center gap-3 min-w-0">
                        ...
                        <span className="text-sm text-site-text truncate">
                          {lesson.title}
                        </span>
                      </span>
        ...
        <div className="mt-10 p-6 bg-site-primary rounded-2xl text-center">
          <p className="text-white font-bold">Ready to start learning?</p>
          ...
          <div className="flex items-center justify-center gap-3 mt-4">
            <Link
              href="/practice"
              className="inline-flex items-center gap-2 bg-site-accent text-site-primary px-5 py-2.5 rounded-xl text-sm font-bold hover:bg-site-accent/90 transition-colors"
            >
              Start free preview
            ...
            <Link
              href="/courses"
              className="inline-flex items-center gap-2 border border-white/20 text-white px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-white/10 transition-colors"
            >
              Browse courses
            </Link>
          </div>
