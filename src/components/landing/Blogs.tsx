...
export default function Blogs({ blogs }: { blogs: Blog[] }) {
  return (
    <section className="py-28 px-6 bg-site-highlight relative overflow-hidden">
      <div className="absolute bottom-0 left-0 w-72 h-72 border border-site-accent/10 rounded-full -translate-x-1/2 translate-y-1/2 pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {blogs.map((blog, index) => (
            ...
            >
              <article className="relative bg-white rounded-2xl border border-site-border overflow-hidden hover:shadow-lg hover:shadow-site-primary/[0.04] hover:-translate-y-1 transition-all duration-300 h-full flex flex-col">
                {/* Image area */}
                <div className="relative h-40 bg-site-primary overflow-hidden">
                  <div
                  ...
                  {/* Decorative circles */}
                  <div className="absolute top-3 right-3 w-12 h-12 rounded-full border border-white/10" />
                  <div className="absolute bottom-3 right-10 w-6 h-6 rounded-full border border-white/10" />
                ...
                <div className="p-5 flex flex-col flex-1">
                  ...
                  {/* Title */}
                  <h3 className="text-base font-bold text-site-text group-hover:text-site-secondary transition-colors leading-snug">
                    {blog.title}
                  </h3>

                  {/* Excerpt */}
                  <p className="mt-2 text-sm text-site-muted leading-relaxed line-clamp-2 flex-1">
                    {blog.content.replace(/<[^>]*>/g, "").substring(0, 120)}
                    {blog.content.length > 120 ? "..." : ""}
                  </p>

                  {/* Read more */}
                  <div className="mt-4 pt-4 border-t border-site-border flex items-center gap-1.5 text-sm font-semibold text-site-secondary group-hover:text-site-primary transition-colors">
                    Read article
